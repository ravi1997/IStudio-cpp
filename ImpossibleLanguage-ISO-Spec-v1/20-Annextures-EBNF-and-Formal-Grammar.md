# 20 — Annexures: Full EBNF (Normative)

```
program         = { module-decl | import-decl | declaration } ;

declaration     = var-decl | const-decl | final-decl | func-decl | class-decl | interface-decl | trait-decl | meta-class-decl | operator-decl | template-decl | concept-decl | contract-func ;
...
contract-clause = "require" expression [ "error" string ] ";" | "ensure" expression [ "error" string ] ";" | "invariant" expression [ "error" string ] ";" ;
...
method          = [ visibility ] [ "static" ] [ "virtual" | "override" | "final" ] "function" identifier "(" [ param-list ] ")" [ ":" type ] [ contract-clauses ] block ;
...
block           = "{" { statement } "}" ;
statement       = block | var-decl | const-decl | final-decl | expr-stmt | if-stmt | match-stmt | while-stmt | repeat-stmt | for-stmt | for-each-stmt | for-each-kv-stmt
...
constexpr-expression = expression ; // restricted to translation-time evaluable subset

// Expressions
expression      = assignment ;
assignment      = ownership-transfer | (identifier "=" expression) | (expression "?" expression ":" expression) ;
...
primary         = identifier | literal | "(" expression ")" | "new" type "(" [ argument-list ] ")" | 
                  ...
                  "match" "(" expression ")" "{" { "case" pattern ":" expression } [ "default" ":" expression ] "}" ;  // match expression
